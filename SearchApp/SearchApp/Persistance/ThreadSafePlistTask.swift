...
public class ThreadSafePlistTask {
    ...
    private struct Constants {
        static let commonOperationQueue = DispatchQueue(label: "com.searchapp.ThreadSafePlistTask.common.reader", qos: DispatchQoS.userInitiated, attributes: DispatchQueue.Attributes.concurrent, autoreleaseFrequency: DispatchQueue.AutoreleaseFrequency.inherit, target: nil)
        static let writerQueue = DispatchQueue(label: "com.searchapp.ThreadSafePlistTask.common.write")
    ...
    @objc private func notificationApplicationWillResignActive() {
        Constants.commonOperationQueue.sync(flags: .barrier) { [unowned self] in
            print("ThreadSafePlistTask notificationApplicationWillResignActive: saving \(String(describing: self.plistPath))")
            self.tryExpnesiveWrite(expnesiveWrite: true)
        }
    }
